    ...
    <section className="section-padding bg-cream">
      <div className="container-grove">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-4xl sm:text-5xl">How an Engagement Works</h2>
        </Reveal>
          ...
          {steps.map((step, index) => (
            <Reveal key={step.step} delayMs={index * 100}>
              <div className="h-full rounded-lg border border-gold/25 bg-cream-100/70 p-8 shadow-card">
                ...
                <h3 className="mt-4 font-heading text-xl font-semibold text-onyx">{step.title}</h3>
                <p className="mt-3 text-base leading-relaxed text-onyx/60">{step.detail}</p>
              </div>
            </Reveal>
          ))}
        </div>
