...
export default function ResultsSpotlight() {
  ...
  return (
    <section className="section-padding border-y border-gold/15 bg-cream-100">
      <div className="container-grove">
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, index) => (
            <Reveal key={stat.id} delayMs={index * 100}>
              <div className="flex h-full flex-col items-center rounded-lg border border-gold/25 bg-cream/70 px-6 py-10 text-center shadow-card backdrop-blur-sm">
                <span className="font-heading text-4xl font-bold text-gold-dark sm:text-5xl">
                  {stat.metric}
                </span>
                <span className="mt-3 text-sm font-medium uppercase tracking-wide text-onyx/60">
                  {stat.metricLabel}
                </span>
                <span className="mt-4 text-xs leading-relaxed text-onyx/80">{stat.district}</span>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delayMs={300} className="mt-12 flex justify-center">
          <Link
            href="/case-studies"
            className="group inline-flex items-center gap-2 text-sm font-semibold text-gold-dark transition-colors hover:text-onyx"
          >
