...
export default function BookingTimeline() {
  ...
  return (
    <section className="border-b border-gold/15 bg-cream px-6 py-16 sm:px-8 lg:px-12 lg:py-20">
      <div className="mx-auto max-w-6xl">
        <span className="eyebrow">{whatHappensNext.eyebrow}</span>
        <h2 className="mt-3 text-2xl text-onyx sm:text-3xl">From First Click to Strategy Call</h2>
          ...
          {whatHappensNext.steps.map((step, index) => {
            ...
            return (
              <div key={step.step} className="flex flex-1 items-center last:flex-none">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-gold/40 bg-cream-200 text-gold-dark">
          ...
          {whatHappensNext.steps.map((step, index) => {
            ...
            return (
              <div key={step.step} className="flex flex-col items-start">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-gold/40 bg-cream-200 text-gold-dark sm:hidden">
                  <Icon size={22} strokeWidth={1.75} />
                </div>
                <span className="mt-4 font-heading text-xs font-bold tracking-wider text-gold-dark sm:mt-0">
                  {step.step}
                </span>
