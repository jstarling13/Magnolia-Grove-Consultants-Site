...
export default function NoirStatStrip({ stats }: NoirStatStripProps) {
  return (
    <section className="border-y border-gold/15 bg-cream-100">
      <div className="mx-auto grid max-w-8xl grid-cols-2 divide-y divide-gold/15 sm:grid-cols-4 sm:divide-x sm:divide-y-0">
        {stats.map((stat) => (
          ...
          >
            <span className="font-heading text-3xl font-bold text-onyx sm:text-4xl">
              {stat.raw ?? (
                <Counter
                  value={stat.value ?? 0}
                  prefix={stat.prefix ?? ""}
            ...
            <span className="text-xs font-medium uppercase tracking-wide text-onyx/60">
              {stat.label}
            </span>
          </div>
        ))}
