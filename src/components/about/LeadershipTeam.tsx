...
function MemberAvatar({ member }: { member: (typeof teamSection.members)[number] }) {
  ...
  return (
    <div className="flex h-28 w-28 shrink-0 items-center justify-center rounded-full border border-gold/40 bg-onyx font-heading text-3xl font-bold text-gold-bright sm:h-32 sm:w-32">
        ...
        {isSingle ? (
          <Reveal className="mx-auto mt-14 max-w-3xl">
            {teamSection.members.map((member) => (
              <div
                key={member.name}
                className="flex flex-col items-center gap-6 rounded-lg border border-gold/25 bg-onyx/70 p-8 text-center backdrop-blur-sm sm:flex-row sm:items-start sm:text-left"
              >
                <MemberAvatar member={member} />
                <div>
                  <h3 className="font-heading text-xl font-semibold text-white">{member.name}</h3>
                  <span className="mt-1 block text-xs font-semibold uppercase tracking-[0.16em] text-gold-bright">
                    {member.role}
                  </span>
